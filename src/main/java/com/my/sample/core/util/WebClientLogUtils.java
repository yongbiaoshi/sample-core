...
public class WebClientLogUtils {
    ...
    public static <T> Mono<T> responseHandle(String requestParams, WebClient.ResponseSpec responseSpec, ParameterizedTypeReference<T> typeReference) {
        return responseSpec
                ...
                // 当调用出错时，记录日志
                .doOnError(ex -> {
                    String errorMsg = "WebClient请求出错，请求参数：%s，错误信息：%s";
                    log.error(String.format(errorMsg, requestParams, ex.getMessage()), ex);
                }).doOnSuccess(t -> {
                    String msg = "WebClient请求成功，请求参数：%s，返回结果：%s";
                    log.debug(String.format(msg, requestParams, Objects.toString(t, "null")));
                });
    }
}
